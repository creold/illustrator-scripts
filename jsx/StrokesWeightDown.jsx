...
function main() {
  ...
  if (ScriptUI.environment.keyboardState.altKey) {
    ...
    for (var i = 0, len = items.length; i < len; i++) {
      decreaseStrokeWeight(items[i], CFG, undefined, undefined);
    }
  }
}
...
function invokeUI(items, CFG) {
  ...
  var isUndo = false;
  var max = 1000;
  var tmpPercent = 50, tmpDelta = -1.5;
...
/**
 * Decrease the stroke width relative to the current stroke width of the item
 *
 * @param {Object} item - The item whose stroke width is to be decreased
 ...
 */
function decreaseStrokeWeight(item, prefs, isPercent, value) {
  if (!isHasStroke(item)) return;
  ...
  if (isPercent !== undefined && value !== undefined) {
    ...
    if (curWidth <= 0.01) {
      newWidth = 0;
    } else if (roundNum(curWidth, 1) <= 0.1) {
      newWidth = (prefs.isRound ? roundNum(curWidth, 2) : curWidth) - 0.01;
    } else if (roundNum(curWidth, 1) <= 1.5) {
      newWidth = (prefs.isRound ? roundNum(curWidth, 1) : curWidth) - (roundNum(curWidth, 1) > 0.2 ? 0.2 : 0.1);
    } else if (curWidth < 5) {
