/*
  ...
  Release notes:
  0.5 Added artboard name labels generation
...
function main() {
  ...
  absInp.onChange = xInp.onChange = yInp.onChange = preview;
  ...
  function saveSettings(prefs) {
    ...
    data.style = charDdl.selection.text;
  ...
  function loadSettings(prefs) {
    ...
    try {
      ...
      if (typeof data != 'undefined') {
        ...
        if (charDdl && data.style) {
          for (var i = 0; i < charDdl.items.length; i++) {
            if (charDdl.items[i].text === data.style) {
              charDdl.selection = i;
              break;
            }
          }
