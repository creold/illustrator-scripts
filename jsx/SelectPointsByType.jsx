...
function main() {
  ...
  var icoBezier = ScriptUI.newImage(BIN_BEZIER_N, BIN_BEZIER_D, BIN_BEZIER_N, BIN_BEZIER_N),
      ...
      icoCorner = ScriptUI.newImage(BIN_CORNER_N, BIN_CORNER_D, BIN_CORNER_N, BIN_CORNER_N),
      icoBroken = ScriptUI.newImage(BIN_BROKEN_N, BIN_BROKEN_D, BIN_BROKEN_N, BIN_BROKEN_N),
      icoFlat = ScriptUI.newImage(BIN_FLAT_N, BIN_FLAT_D, BIN_FLAT_N, BIN_FLAT_N);
  ...
  var sPoints = dialog.add('statictext', undefined);
      sPoints.text = 'Selected Points: ' + getSelectedPoints(selPaths);
      sPoints.justify = 'center';
  ...
  dialog.addEventListener('keydown', function(kd) {
    ...
    if (kd.altKey) {
      ...
      if (kd.keyName.match(/2/)) flushBtn.notify();
      if (kd.keyName.match(/3/)) cornerBtn.notify();
      if (kd.keyName.match(/4/)) brokenBtn.notify();
      if (kd.keyName.match(/5/)) flatBtn.notify();
    }
  });
...
function getPointType(point, tolerance) {
  ...
  if (isZeroLHandle && isZeroRHandle) return 'flat';
  if (isZeroLHandle) return 'l_zero';
...
// Get coordinates anchor & hangles
  ...
  for (var i = 0; i < paths.length; i++) {
    if (paths[i].pathPoints.length > 1) {
      ...
      for (var j = 0; j < points.length; j++) {
        ...
        if (paths[i].closed) {
          if (j == 0) {
            coordArr = getCoordinates(points[1], currPoint, points[points.length - 1], answer);
            xArr = coordArr.x;
            yArr = coordArr.y;
          }
          if (j == points.length - 1) {
            coordArr = getCoordinates(points[0], currPoint, points[points.length - 2], answer);
            xArr = coordArr.x;
            yArr = coordArr.y;
          }
        }
        ...
        if (j > 0 && j < points.length - 1) {
          coordArr = getCoordinates(points[j + 1], currPoint, points[j - 1], answer);
          xArr = coordArr.x;
          yArr = coordArr.y;
        };
        ...
        // Bezier Points
        if (btnsVal[0] && answer == 'bezier') {
          selector(currPoint);
        }
        // Flush Points
        if (btnsVal[1] && (answer == 'l_zero' || answer == 'r_zero') && (pointCosRound == COS_180 || pointCos < COS_INACCURACY)) {
          selector(currPoint);
        }
        // Corner Points
        if (btnsVal[2] && (answer == 'flat' || answer == 'r_zero' || answer == 'l_zero' || answer == 'corner') 
             && pointCosRound !== COS_180 && (isNaN(pointCos) || pointCos > COS_INACCURACY)) {
          selector(currPoint);
        }
        // Broken Points
        if (btnsVal[3] && answer == 'broken') {
          selector(currPoint);
        }
        // Flat Points
        if (btnsVal[4] && answer == 'flat' && (pointCosRound == COS_180 || pointCos < COS_INACCURACY)) {
          selector(currPoint);
        }
      }
    }
