/*
  ...
  Versions:
   ...
   1.1.1 Fixed folder path for icons
  ============================================================================
...
// Create & get icons files
var SMOOTH_ICNS_PNG = createIcons(SMOOTH_ICNS_RAW),
    CORNER_ICNS_PNG = createIcons(CORNER_ICNS_RAW),
    BROKEN_ICNS_PNG = createIcons(BROKEN_ICNS_RAW);
...
function main() {
  ...
  var smoothIcns = ScriptUI.newImage(SMOOTH_ICNS_PNG[0], SMOOTH_ICNS_PNG[1], SMOOTH_ICNS_PNG[0], SMOOTH_ICNS_PNG[0]),
      cornerIcns = ScriptUI.newImage(CORNER_ICNS_PNG[0], CORNER_ICNS_PNG[1], CORNER_ICNS_PNG[0], CORNER_ICNS_PNG[0]),
      brokenIcns = ScriptUI.newImage(BROKEN_ICNS_PNG[0], BROKEN_ICNS_PNG[1], BROKEN_ICNS_PNG[0], BROKEN_ICNS_PNG[0]);
  ...
  toleranceVal.onChanging = function() {
    ...
    if (convertDecimalPoint(this.text) < MIN_ANGLE) this.text = MIN_ANGLE;
    if (convertDecimalPoint(this.text) > MAX_ANGLE) this.text = MAX_ANGLE;

    if (isNaN(convertDecimalPoint(this.text))) {
  ...
  function run() {
    ...
    selectPoints(btnsState, selPaths, convertDecimalPoint(toleranceVal.text));
  }
...
function createIcons(obj) {
  var outfile,
      arr = [];
  for (var key in obj) {
    var filePath = ICNS_FOLDER + encodeURI(key) + '.png';
    try {
